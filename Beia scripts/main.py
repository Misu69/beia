...
class SensorSimulator:
    def __init__(self, broker_host="localhost", broker_port=1883, topic="sensors/data", ganache_url=None, sender_wallet=None, receiver_wallet=None, offline_file='./mihai-lazar/offline-data.json'):
        ...
        self.setup_mqtt()
        self.offline_mode = False  # Track offline mode explicitly
        ...
        if self.ganache_url:
            ...
            if not self.w3.is_connected():
                ...
                self.offline_mode = True  # Set offline mode if Ganache connection fails
            else:
                logger.info("Connected to Ganache")
        
        # Use absolute path for the offline file and ensure directory exists
    ...
    def generate_temperature(self):
        ...
        # Base temperature around 20-25°C with some variation
        base_temp = 22.5
    ...
    def generate_humidity(self):
        ...
        # Humidity between 30-80% with normal distribution around 50%
    ...
    def send_to_blockchain(self, data, allow_offline_save=True):
        ...
        try:
            tx_hash = self.w3.eth.send_transaction({
                ...
                "value": self.w3.to_wei(0, "ether"), # No ether sent, just data
    ...
    def save_data_offline(self, data):
        ...
        try:
            # Make sure we're saving to an absolute path
    ...
    def run_simulation(self, interval=10, duration=None):
        ...
        if not connected:
            ...
            # Force to save data offline since we know we're offline
        ...
        try:
            ...
            while True:
                ...
                # Try to send data, but if we're already in offline mode, just save it directly
                if self.offline_mode:
                    ...
                    # If both failed, we'll save the data only once (instead of twice)
...
def main():
    # Configuration
    MQTT_BROKER = "mqtt.beia-telemetrie.ro"  # External MQTT broker
    MQTT_PORT = 1883
    ...
    # Run simulation (sends data every 10 seconds)
